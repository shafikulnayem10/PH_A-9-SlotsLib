...
export function Testimonials() {
  return (
    <section className="bg-white text-slate-900 py-20 px-4 md:px-8 relative overflow-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16 space-y-3">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-orange-50 rounded-full border border-orange-100">
            <span className="text-xs font-bold text-orange-600 uppercase tracking-wider">Reviews</span>
          </div>
        ...
        <div className="relative px-12">
          ...
          >
            {reviewsData.map((review) => (
              <SwiperSlide key={review.id} className="!h-auto flex"> 
                <div className="bg-slate-50/60 border border-slate-100 rounded-3xl p-8 flex flex-col justify-between w-full min-h-[320px] transition-all duration-300 hover:shadow-xl hover:border-orange-500/20 group">
                  ...
                  <p className="text-slate-600 text-sm md:text-base font-medium leading-relaxed text-center flex-grow mb-6">
                    "{review.comment}"
                  </p>

               
                  <div className="mt-auto text-center border-t border-slate-100 pt-4 shrink-0">
                    <h4 className="text-orange-600 font-extrabold text-base group-hover:text-orange-500 transition-colors">
                      - {review.name}
                    </h4>
                    <span className="text-slate-400 font-semibold text-xs uppercase tracking-wider block mt-0.5">
                      {review.company}
                    </span>
                  </div>
                </div>
          ...
          <button className="custom-prev absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white hover:bg-orange-500 text-slate-700 hover:text-white p-3 rounded-xl transition-all shadow-md hover:shadow-orange-500/20 border border-slate-100 focus:outline-none">
            <ChevronLeft className="w-5 h-5" />
          </button>
          
          <button className="custom-next absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white hover:bg-orange-500 text-slate-700 hover:text-white p-3 rounded-xl transition-all shadow-md hover:shadow-orange-500/20 border border-slate-100 focus:outline-none">
